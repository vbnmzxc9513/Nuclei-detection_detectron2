...
setup_logger()
# registet_coco_dataset
register_coco_instances("my_dataset", {},
                        "stage1_train_annotations.json", "./stage1_train")
# to get matadata for visualization
...
cfg = get_cfg()
# use cascade_mask_rcnn_R50_FPN for trainging model config
cfg.merge_from_file("./model/cascade_mask_rcnn_R_50_FPN_3x.yaml")
cfg.OUTPUT_DIR = "./output_Cascade"  # output weight directroy path
# the path for weight save
cfg.MODEL.WEIGHTS = "./output_Cascade/model_0004999.pth"
...
im = cv2.imread(
    "./stage1_test/472b1c5ff988dadc"
    + "209faea92499bc07f305208dbda29d16262b3d543ac91c71.png")
...
for weight in fold:
    ...
    cfg.SOLVER.IMS_PER_BATCH = 2
    cfg.SOLVER.BASE_LR = 0.00025
    # 30000 iterations seems good enough, but you can certainly train longer
    ...
    df.to_csv("./output_flip_csv/R50_flip_" + weight +
              ".csv", index=False, sep=',')
